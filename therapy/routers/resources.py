...
from queries.resources import ResourceIn, ResourceRepository, ResourceOut, Error

router = APIRouter()


@router.post("/resources", response_model=Union[ResourceOut, Error])
def create_resource(resource: ResourceIn, response: Response, repo: ResourceRepository = Depends()):
...
def get_resource(resource_id: int, repo: ResourceRepository = Depends()) ->bool:
        return repo.get_resource(resource_id)

@router.delete("/resources/{resource_id}", response_model=bool)
def delete_resource(resource_id: int, repo: ResourceRepository = Depends()) ->bool:
        return repo.delete_resource(resource_id)

@router.put("/resources/{resource_id}", response_model=Union[Error, ResourceOut])
def update_resource(resource_id: int, resource: ResourceIn, repo: ResourceRepository = Depends()
