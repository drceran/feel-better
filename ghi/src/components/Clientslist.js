...
function ClientsList() {
    const { id } = useParams();
    const { tokenData, errorToken } = useGetTokenQuery();
    const { data, isLoading } = useGetClientsQuery(tokenData?.account.id);
    const { clientDetailData, clientDetailIsLoading } = useGetClientDetailQuery(tokenData?.account.id);
